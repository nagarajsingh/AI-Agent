...
from ai_extractor import ai_extract_services_from_text, ai_extract_code_pull_details
from azure_devops import resolve_pipeline_id, trigger_image_pipeline, trigger_code_pull_pipeline, get_run_status
from database import init_db, insert_deployment, get_deployments_by_date, update_build_status
from pdf_utils import extract_services_from_pdf, extract_text_from_pdf
...
def check_config():
    required = ["AZDO_ORG", "AZDO_PROJECT", "AZDO_PAT", "DB_HOST", "DB_NAME", "DB_USER", "DB_PASSWORD"]
...
def trigger_service(service, vendor_image, environment, use_vendor_image):
    ...
    try:
        data = trigger_image_pipeline(pipeline_id, vendor_image, use_vendor_image)
        return {
            "Service": service,
            "vendorImage": vendor_image,
            "Environment": environment,
...
extraction_type = st.radio("Extraction Type", ["Image Tag Extraction", "Code Pull Extraction"], horizontal=True)
...
if uploaded_file and extraction_type == "Code Pull Extraction":
    ...
    if "code_pull_json" in st.session_state:
        ...
        try:
            ...
            st.caption("Azure DevOps parameter names are configured using CODE_PULL_PARAM_* environment variables.")
            if st.button("Trigger Code Pull Pipeline"):
                try:
                    result = trigger_code_pull_pipeline(validated_json)
                    st.success("Code-pull pipeline triggered successfully.")
                    st.json(result)
                except Exception as exc:
                    st.error(f"Failed to trigger code-pull pipeline: {str(exc)}")
        except Exception as exc:
    ...
    if not extracted_data and use_ai_extractor:
        ...
        try:
            ...
            if not document_text:
                st.warning("Could not extract text from PDF. AI extraction skipped.")
            else:
        ...
        if st.button("Trigger Selected Service Pipelines"):
            if selected_df.empty:
                ...
                for _, row in selected_df.iterrows():
                    result = trigger_service(row["Service"], row["vendorImage"], environment, use_vendor_image)
...
if dashboard_df.empty:
    ...
    if st.button("Retrigger Selected Services"):
        ...
        if selected_retry_df.empty:
            ...
            for _, row in selected_retry_df.iterrows():
                result = trigger_service(row["service_name"], row["vendor_image"], row["environment"], use_vendor_image)
                insert_deployment(result)
    ...
    if not build_view.empty:
        build_view = build_view.rename(columns={"service_name": "Service", "run_id": "Build Run", "build_state": "State", "build_result": "Result", "build_url": "Link"})
        st.dataframe(build_view, use_container_width=True, column_config={"Link": st.column_config.LinkColumn("Link", display_text="Open")})
    else:
